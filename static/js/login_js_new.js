...
come_in_btn1.onclick = function () {
    ...
    EnterData.name_com = document.getElementById("name_com").value;
    EnterData.pas_com = document.getElementById("pas_com").value;
    if (EnterData.name_com == "" || EnterData.pas_com == "") {
        alert("Заполните все поля");
    } else {
        $.ajax({
            type: "POST",
            url: "/come_in",
            data: {
                name_com: EnterData.name_com,
                pas_com: EnterData.pas_com
            },
            type: 'POST',
            success: function (response) {
